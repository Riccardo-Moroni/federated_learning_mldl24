...
for k in range(K):
    ...
    X_k_val = X_val_tensor[k]
    Y_k_val = Y_val_tensor[k]
    train_datasets.append(TensorDataset(X_k, Y_k))
    val_datasets.append(TensorDataset(X_k_val, Y_k_val))
...
def client_update(model, k, params):
    ...
    val_loader = DataLoader(val_datasets[k], batch_size=params['B'], shuffle=False)
    ...
    for i in range(params['J']):
        for batch_idx, (inputs, targets) in enumerate(train_loader):
            ...
            if i >= params['J']:
                ...
                return model.state_dict(), client_accuracy, client_loss, *test(model, val_loader)
...
def train(model, params):
    ...
    for t in range(params['rounds']):
        ...
        w_val_losses = []
        for k in s:
            update, client_accuracy, client_train_loss, client_val_acc, client_val_loss = client_update(copy.deepcopy(model), k, params)
